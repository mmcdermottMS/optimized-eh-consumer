...
aggregate_function_bp = func.Blueprint() 

configure_azure_monitor()

@aggregate_function_bp.route(route="aggregate") 
async def aggregate(req: func.HttpRequest, context) -> func.HttpResponse:
    ...
    with tracer.start_as_current_span("aggregate_items", context=extract(carrier)): #Give the span a unique name, it will show up in App Insights as a parent dependency to all calls made within the span
        
        return func.HttpResponse(f"Aggregation Function Executed", status_code=200)